...
loss_unl =  0.5*(CT_ - T.mean(l_unl) + T.mean(T.nnet.softplus(nn.log_sum_exp(output_before_softmax_unl))) + T.mean(T.nnet.softplus(nn.log_sum_exp(output_before_softmax_fake)))) # too much noise, to keep the structure the same, weight should be smaller
...
train_batch_disc = th.function(inputs=[x_lab,labels,x_unl,lr], outputs=[loss_lab, loss_unl, train_err,loss_ct], updates=disc_param_updates+disc_avg_updates)
...
for epoch in range(300):  # 300 epochs
    ...
    for t in range(nr_batches_train):
        ll, lu, te,tc = train_batch_disc(trainx[t*args.batch_size:(t+1)*args.batch_size],trainy[t*args.batch_size:(t+1)*args.batch_size],
