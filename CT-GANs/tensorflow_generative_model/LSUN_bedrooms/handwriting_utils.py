"""
from http://github.com/hardmaru/write-rnn-tensorflow
"""
...
def draw_many_strokes(data_batch, filename, factor=20):
  doc_x = 0
  doc_y = 0
  ...
  for data in data_batch:
    ...
    doc_x = max(doc_x, dims[0])
    doc_y += dims[1]

  dwg = svgwrite.Drawing(filename, size=(doc_x, doc_y))
  dwg.add(dwg.rect(insert=(0, 0), size=(doc_x, doc_y),fill='white'))

  y_offset = 0
  for data_i, data in enumerate(data_batch):  
    min_x, max_x, min_y, max_y = get_bounds(data, factor)

    lift_pen = 1
      
    abs_x = 25 - min_x 
    abs_y = 25+y_offset - min_y
    p = "M%s,%s " % (abs_x, abs_y)
...
class DataLoader():
  def __init__(self, batch_size=50, seq_length=300, scale_factor = 10, limit = 500):
    # self.data_dir = "/media/ramdisk/handwriting"
    self.data_dir = "/home/ishaan/data/handwriting"
